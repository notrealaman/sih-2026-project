...
export default function Recommend() {
  ...
  if (!profile) return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-4 px-4">
      ...
      <Link to="/student/assess" className="btn-primary">Start Assessment</Link>
    </div>
  )
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b border-slate-200">
        <div className="container py-6">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Matched Opportunities</h1>
          <p className="text-sm text-slate-500 mt-1">{jobs.length} positions ranked by skill compatibility</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          {sorted.length === 0 && (
            <div className="card-elevated p-12 text-center">
              <p className="text-slate-500">No positions match the current filter.</p>
            </div>
          )}
        </div>
      </div>
