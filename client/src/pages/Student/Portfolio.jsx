...
function RadarChart({ skills, size = 280 }) {
  ...
  const pathD = skillPoints.map((p, i) => `${i === 0 ? 'M' : 'L'}${p.x},${p.y}`).join(' ') + 'Z'
    ...
    <div className="flex items-center gap-4 py-3 border-b border-slate-100 last:border-0 animate-slide-up" style={{ animationDelay: `${index * 0.03}s` }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="progress-track">
          <div className={`progress-fill ${levelColor}`} style={{ width: `${skill.level}%`, animationDelay: `${index * 50}ms` }} />
        </div>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="container py-6">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Clinical Skill Profile</h1>
          <p className="text-sm text-slate-500 mt-1">Assessment results and competency mapping</p>
        </div>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="card-elevated p-6">
              ...
              <div className="flex justify-center">
                <RadarChart skills={profile.profile.slice(0, 10)} />
              </div>
            </div>
            ...
            <div className="card-elevated p-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-slate-900">{profile.profile.length}</div>
                  <div className="text-xs text-slate-500 mt-0.5">Skills</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">{profile.topSkills.length}</div>
                  <div className="text-xs text-slate-500 mt-0.5">Strengths</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-amber-600">{profile.gapSkills.length}</div>
            ...
            <div className="card-elevated p-6">
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {profile.topSkills.length ? profile.topSkills.map(s => (
                  <span key={s.id} className="badge badge-green text-[10px]">{s.name}</span>
              ...
              <div className="flex flex-wrap gap-1.5">
                {profile.gapSkills.length ? profile.gapSkills.map(s => (
                  <span key={s.id} className="badge badge-yellow text-[10px]">{s.name}</span>
