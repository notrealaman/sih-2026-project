    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="max-w-5xl mx-auto p-4 sm:p-8 grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 -mt-4">
        <div className="lg:col-span-2 space-y-3">
          <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><span className="w-2 h-2 bg-indigo-500 rounded-full" />Skill Profile</h2>
          {profile.profile.map((skill, i) => (
            <div key={skill.id} className="bg-gray-900 rounded-2xl p-4 sm:p-5 border border-gray-800 hover:border-gray-700 transition-colors animate-fade-in-up" style={{ animationDelay: `${i * 0.05}s` }}>
              ...
              <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                <div className={`h-full rounded-full bg-gradient-to-r ${levelColor(skill.level)} transition-all duration-700`} style={{ width: `${skill.level}%` }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-gray-900 rounded-2xl p-5 sm:p-6 border border-gray-800">
            <h3 className="font-bold text-white mb-4 flex items-center gap-2"><span className="text-green-400">✓</span> Top Strengths</h3>
            <div className="flex flex-wrap gap-2">
              {profile.topSkills.length ? profile.topSkills.map(s => (
                <span key={s.id} className="bg-green-500/10 border border-green-500/20 text-green-400 text-sm font-semibold px-3 py-1.5 rounded-xl">{s.name}</span>
              )) : <p className="text-gray-500 text-sm">None yet</p>}
            </div>
          </div>
          <div className="bg-gray-900 rounded-2xl p-5 sm:p-6 border border-gray-800">
            <h3 className="font-bold text-white mb-4 flex items-center gap-2"><span className="text-orange-400">!</span> Skill Gaps</h3>
            <div className="flex flex-wrap gap-2">
              {profile.gapSkills.length ? profile.gapSkills.map(s => (
                <span key={s.id} className="bg-orange-500/10 border border-orange-500/20 text-orange-400 text-sm font-semibold px-3 py-1.5 rounded-xl">{s.name}</span>
              )) : <p className="text-green-400 text-sm font-medium">No major gaps!</p>}
            </div>
          </div>
          <Link to="/student/recommend" className="block bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-center py-4 rounded-2xl font-bold hover:scale-[1.02] transition-all shadow-lg shadow-indigo-500/25">
            View Recommendations →
          </Link>
          {user && <Link to="/dashboard" className="block bg-gray-900 text-gray-400 text-center py-3 rounded-2xl font-medium hover:text-white hover:bg-gray-800 transition-all border border-gray-800">My Applications →</Link>}
        </div>
      </div>
    </div>
  )
