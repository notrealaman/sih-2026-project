...
export default function Industry() {
  ...
  const filtered = jobs.filter(j => {
    ...
    const matchSearch = !search || j.title.toLowerCase().includes(search.toLowerCase()) || j.org_name.toLowerCase().includes(search.toLowerCase())
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b border-slate-200">
        <div className="container py-6">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Healthcare Positions</h1>
          <p className="text-sm text-slate-500 mt-1">{filtered.length} open roles across healthcare organizations</p>
        </div>
      </div>
      ...
      <div className="container py-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
          ...
          <div className="tab-group">
            {types.map(t => (
              <button key={t} onClick={() => setFilter(t)} className={`tab ${filter === t ? 'tab-active' : ''}`}>
                {t === 'all' ? 'All' : t}
              </button>
            ))}
          </div>
          ...
          {filtered.map((job, i) => (
            <div key={job.id} className="card-elevated p-5 animate-slide-up" style={{ animationDelay: `${i * 0.03}s` }}>
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                <div className="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center text-slate-600 font-bold text-sm flex-shrink-0">
                  {job.org_name?.[0] || 'H'}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {job.required_skills.slice(0, 6).map(s => (
                      <span key={s} className="text-[10px] font-medium px-2 py-0.5 rounded bg-slate-100 text-slate-500">{s}</span>
                    ))}
                ...
                <div className="shrink-0">
                  {applied.has(job.id) ? (
                    ...
                    <button onClick={() => handleApply(job.id)} className="btn-primary text-sm py-2 px-4">Apply</button>
                  )}
                </div>
              </div>
            </div>
